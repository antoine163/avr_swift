...
ISR(ADC_vect)
{
	unsigned int tmpadc = adc_get_conversion_10bit(); 		//Récupération la valeur de convection sur 10 bits.

	// ...

	tc_cleared_overflow_flag(&tc0);				//On affecte le drapeau de débordement du timer pour pouvoir refaire d'autre conversion.
}
...
int main(void)
{
	...
	adc_disable_input(ADC_ADC0);				//Désactiver l'entrée numérique de la pin ADC0. (Pas obligatoire).
